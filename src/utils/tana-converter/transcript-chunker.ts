...
function findBestSplitPoint(text: string, targetPosition: number): number {
  ...
  const sentenceMatches = Array.from(searchText.matchAll(CHUNKING_CONSTANTS.SENTENCE_BOUNDARY_PATTERN))
  ...
  for (const word of words) {
    ...
    if (Math.abs((searchStart + wordCenter) - targetPosition) < 
...
export function chunkTranscriptContent(transcript: string): TranscriptChunk[] {
  ...
  while (currentPosition < transcriptLength) {
    ...
    if (chunkContent.length < CHUNKING_CONSTANTS.MIN_CHUNK_SIZE && 
        actualEnd < transcriptLength) {
    ...
    if (!isValidTranscriptChunk(chunk)) {
      throw new ChunkingError(`Generated invalid chunk at position ${currentPosition}`, {
        chunkNumber,
        chunkContentType: typeof (chunk as any)?.content,
        chunkContentLength: getSafeLength((chunk as any)?.content)
      })
    }
    ...
    const nextPosition = actualEnd - CHUNKING_CONSTANTS.OVERLAP_SIZE
...
export function generateTranscriptOutput(chunks: TranscriptChunk[]): string {
  ...
  try {
    ...
    if (chunks.length === 1) {
      ...
      builder.addLine(`Transcript (${chunks.length} parts, ${totalWords} words, ~${totalDuration})`, 0)
...
export function generateHierarchicalTranscriptOutput(
  ...
  try {
    ...
    if (chunks.length === 1) {
      ...
      builder.addLine(`Transcript (${chunks.length} parts, ${totalWords} words, ~${totalDuration})`, transcriptIndent)
